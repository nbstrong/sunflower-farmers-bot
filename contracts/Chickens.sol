...
contract Chicken is ERC20, ERC20Burnable {
    ...
    mapping(address => uint256) _hatchedAt;

    constructor(address coop, address egg)
        payable
        ERC20("Sunflower Land Chicken", "SLC")
    {
        _coop = coop;
        _minter = msg.sender;
        _egg = egg;
    ...
    function hatchTime(address account) public returns (uint256) {
        return _hatchedAt[msg.sender];
    }

    function collectEggs() public {
        uint256 chickens = super.balanceOf(msg.sender);
        require(chickens > 0, "You have no chickens");
        require(
            block.timestamp - _hatchedAt[msg.sender] > 60 * 60 * 24,
            "You have to wait 24 hours before you can collect eggs"
        );

        // It is actually ERC721 but same principle
        uint256 coop = ERC20(_coop).balanceOf(msg.sender);
